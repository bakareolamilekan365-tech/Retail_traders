...
describe("PriceChart", () => {
  ...
  it("renders chart and overlays", async () => {
    ...
    const addSeries = vi
      ...
      .mockImplementationOnce(() => ({
        setData: sma14SetData,
        update: vi.fn(),
      }))
      .mockImplementationOnce(() => ({
        setData: sma50SetData,
        update: vi.fn(),
      }));
    ...
    expect(sma50SetData).toHaveBeenCalledWith([
      { time: "2024-01-02", value: 2.5 },
    ]);
    expect(
      screen.getByRole("button", { name: /replay history/i }),
    ).toBeInTheDocument();
  ...
  it("steps through historical candles during replay", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /pause replay/i }),
    ).toBeInTheDocument();
    ...
    expect(sma50Update).not.toHaveBeenCalledWith({
      time: "2024-01-01",
      value: null,
    });
  });
