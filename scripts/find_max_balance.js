...
let curly=0, paren=0, maxCurly=0, maxLine=0;
for(let i=0;i<lines.length;i++){
  ...
  if(curly>maxCurly){ maxCurly=curly; maxLine=i+1; }
}
...
for(let i=Math.max(0,maxLine-6); i<Math.min(lines.length, maxLine+6); i++){
